...
import com.fwtai.web.DeviceSoundDao;
...
*/
@Service
public class DeviceSoundService{

    private final Logger logger = LoggerFactory.getLogger(getClass());

    @Resource
    private DeviceSoundDao devicesoundDao;
    ...
    public String add(final HttpServletRequest request){
        ...
        return ToolClient.executeRows(devicesoundDao.add(formData));
    }
    ...
    public String edit(final HttpServletRequest request){
        ...
        if(validate != null)return validate;
        final String exist_key = devicesoundDao.queryExistById(formData.getString(p_kid));
        if(exist_key == null){
            return ToolClient.createJson(ConfigFile.code199,"数据已不存在,刷新重试");
        }
        return ToolClient.executeRows(devicesoundDao.edit(formData));
    }
    ...
    public String queryById(final PageFormData pageFormData){
        ...
        if(validate != null)return validate;
        return ToolClient.queryJson(devicesoundDao.queryById(pageFormData.getString(p_id)));
    }
    ...
    public String delById(final PageFormData formData){
        ...
        final String exist_key = devicesoundDao.queryExistById(kid);
        if(exist_key == null){
            return ToolClient.createJson(ConfigFile.code199,"删除失败,数据已不存在");
        }
        return ToolClient.executeRows(devicesoundDao.delById(kid));
    }
    ...
    public String delByKeys(final PageFormData formData){
        ...
        return ToolClient.executeRows(devicesoundDao.delByKeys(lists),"操作成功","数据已不存在,刷新重试");
    }
